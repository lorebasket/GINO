...
def transform_matrices(file_name, K, M, beam_length, sonata, flutter_benchmark):
    """
    Transform stiffness and mass matrices from Natural Axis to Shear Center.
    
    Args:
        file_name: Path to the section properties CSV file
        K: Stiffness matrix (numpy array) or dictionary of stiffness matrices
        M: Mass matrix (numpy array) or dictionary of mass matrices
        beam_length: Length of the beam
        sonata: Boolean flag for SONATA-specific processing
        flutter_benchmark: Boolean flag for flutter benchmark mode
        
    Returns:
        Tuple of (K_transformed, M_transformed) - transformed stiffness and mass matrices
        in the same format as input (array or dict)
    """
    import numpy as np
    ...
    if sonata:
        ...
        # For benchmark, use zero offsets (no translation)
        NA_X = np.zeros(len(eta_keys))
        NA_Y = np.zeros(len(eta_keys))
        NA_Z = np.zeros(len(eta_keys))
        SC_X = np.zeros(len(eta_keys))
        SC_Y = np.zeros(len(eta_keys))
        SC_Z = np.zeros(len(eta_keys))
...
def rotate_matrices_by_angle(K, M, axis, angle, section_keys=None):
    """
    Rotate stiffness and mass matrices by a given angle about a specified axis.
    
    Parameters:
    - K: dictionary of 6x6 stiffness matrices
    - M: dictionary of 6x6 mass matrices  
    - axis: rotation axis ('x', 'y', 'z' or 3D unit vector)
    - angle: rotation angle in radians
    - section_keys: list of section keys to rotate (if None, rotates all)
    
    Returns:
    - K_rotated: rotated stiffness matrices
    - M_rotated: rotated mass matrices
    """

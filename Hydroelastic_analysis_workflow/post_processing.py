# FSI/Hydroelastic_analysis_workflow/post_processing.py

from panelaero_utl.pk_method_utl import vgvf_plotting
...
def plot_vg_vf(flutter_results, config=None, save_path="", show_plot=False):
    ...
    combined_outfile = f"{save_path}vg_vf_combined.png" if save_path else "vg_vf_combined.png"
    ...
    # Save CSV files
    vg_csv = f"{save_path}vg_data.csv" if save_path else "vg_data.csv"
    vf_csv = f"{save_path}vf_data.csv" if save_path else "vf_data.csv"
    summary_csv = f"{save_path}flutter_summary.csv" if save_path else "flutter_summary.csv"
...
def plot_qmodal_entries_at_convergence(config, structural_results, coupling_results, flutter_results,
    ...
    if save_csv:
        ...
        csv_file = f"{save_path}qmodal_entries.csv" if save_path else "qmodal_entries.csv"
    ...
    plt.tight_layout()
    outfile = f"{save_path}qmodal_entries.png" if save_path else "qmodal_entries.png"
...
def plot_damping_debug(structural_results, flutter_results, config=None):
    ...
    outfile = f"{save_path}damping_debug.png" if save_path else "damping_debug.png"
    plt.savefig(outfile, dpi=150, bbox_inches='tight')
...
def plot_damping_matrices(structural_results, coupling_results, flutter_results, config=None, mode_index=0, velocity='flutter'):
    ...
    if config and getattr(config, 'save_plots', False):
        os.makedirs(config.output_dir, exist_ok=True)
        save_path = config.output_dir
    outfile = f"{save_path}damping_matrices_V{V_samp:.1f}.png" if save_path else f"damping_matrices_V{V_samp:.1f}.png"
    plt.savefig(outfile, dpi=150, bbox_inches='tight')
...
def plot_eigenvalue_trajectory(flutter_results, results_raw, config=None, save_path=""):
    ...
    outfile = f"{save_path}eigenvalue_trajectory.png" if save_path else "eigenvalue_trajectory.png"
    ...
    eigenvalue_csv = f"{save_path}eigenvalue_data.csv" if save_path else "eigenvalue_data.csv"
...
def plot_cp_at_velocities(config, aerogrid, results):
    ...
    for idx in indices:
        ...
        if len(modes_data) > 0:
            ...
            outfile_real = f"{config.output_dir}cp_real_V{V:.2f}.png" if config.save_plots else None
            ...
            outfile_imag = f"{config.output_dir}cp_imag_V{V:.2f}.png" if config.save_plots else None
...
def plot_cp_at_fixed_k_values(config, aerogrid, results, k_values=[0.001, 0.5, 1.0]):
    ...
    for k in k_values:
        ...
        # Save figure
        if config.save_plots:
            outfile = f"{config.output_dir}cp_k{k:.3f}_allmodes.png"
...
def plot_aero_beam_model(aerogrid, beam_model, config=None):
    ...
    if config and config.save_plots:
        output_dir = config.output_dir
        os.makedirs(output_dir, exist_ok=True)
        outfile = f"{output_dir}aero_beam_model.png"
        plt.savefig(outfile, dpi=300, bbox_inches='tight')
...
def plot_stiffness_damping_contributions(pk_solver, config=None, save_path="", show_plot=False):
    ...
    contrib_csv = f"{save_path}stiffness_damping_contributions.csv" if save_path else "stiffness_damping_contributions.csv"
    ...
    # Save figure
    outfile = f"{save_path}stiffness_damping_contributions.png" if save_path else "stiffness_damping_contributions.png"
    plt.savefig(outfile, dpi=150, bbox_inches='tight')
...
def plot_w_modal_components(pk_solver, config=None, save_path=""):
    ...
    w_modal_csv = f"{save_path}w_modal_components.csv" if save_path else "w_modal_components.csv"
    ...
    # Save figure
    outfile = f"{save_path}w_modal_components.png" if save_path else "w_modal_components.png"
    plt.savefig(outfile, dpi=150, bbox_inches='tight')
...
def plot_deformed_mode_shapes(aerogrid, beam_model, structural_results, coupling_results, 
    ...
    for mode_idx in modes_to_plot:
        ...
        if not animate:
            ...
            if config and config.save_plots:
                os.makedirs(config.output_dir, exist_ok=True)
                outfile = f"{config.output_dir}mode_shape_{mode_idx}_deformed.png"
                plt.savefig(outfile, dpi=300, bbox_inches='tight')
            ...
            if config and config.save_plots:
                os.makedirs(config.output_dir, exist_ok=True)
                outfile = f"{config.output_dir}mode_shape_{mode_idx}_animated.gif"

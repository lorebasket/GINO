...
job_str = 'm2_scaled.yaml'  # note: for better meshing convergence, units specified in yaml are in 'mm' instead of 'm'
job_name = 'm2_scaled'
...
with open(section_csv_path, "w", newline="") as csvfile:
    ...
    for eta, data in section_data.items():
        ...
        # where pos = [X, Y, Z] is the beam reference axis position
        # and sc = [sc_x, sc_y] are local coordinates in the section frame from VABS

...
def find_BSplineLst_coordinate(BSplineLst, S, start, end):
    ...
    for i, item in enumerate(BSplineLst):
        ...
        if x < CummLength or isclose(x, CummLength):
            ...
            break
    if U is None:
        # If U is still None, the point was not found on any curve segment.
        raise ValueError(f"Coordinate could not be found on the provided BSpline list within the specified range.")

...
job_str = '2_solid_steel.yaml'  # note: for better meshing convergence, units specified in yaml are in 'mm' instead of 'm'
job_name = '2_solid_steel'
...
with open(section_csv_path, "w", newline="") as csvfile:
    ...
    for eta, data in section_data.items():
        ...
        # Desired mapping (user):
        # SC_global_X = X
        # SC_global_Y = Y + sc_x
        # SC_global_Z = Z + sc_y
        # where pos = [X, Y, Z] and sc = [sc_x, sc_y]

        scx_global = pos[0]
        scy_global = pos[1] + sc[0]
        scz_global = pos[2] + sc[1]
        sc_global = np.array([scx_global, scy_global, scz_global])
        
        # Calculate global coordinates for center of gravity (unchanged)

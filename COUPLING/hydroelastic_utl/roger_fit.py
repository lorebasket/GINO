def load_rfa_matrices_from_disk(config, velocity, space_type):
    ...
    config : AnalysisConfig
        Configuration object containing case information (config.name, config.paths['output_data'])
...
def save_rfa_matrices_to_csv(Q0, Q1, Q2, Alag, blag, config, output_dir, velocity, space_type="panel"):
    ...
    # Create output directory structure: output_data/RFA_matrices/{config.name}/V_{velocity}/{space_type}/
    space_label = "panel_space" if space_type == "panel" else "rigid_body_space"
...
def main():
    ...
    for V in Vlist:
        ...
        save_rfa_matrices_to_csv(Q0, Q1, Q2, Alag, blag, _analysis_cfg, FSI_path + '/output_data', V, space_type="panel")

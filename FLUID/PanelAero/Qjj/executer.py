...
def main(start_time=None):
    ...
    FSI_path = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
    ...
    for n_span in nspan:
        for aspect_ratio in AR:
            ...
            if method == 1:
                ...
                output_dir = FSI_path + '/FLUID/PanelAero/Qjj/CAERO1_cards'
            ...
            if method == 2:
                ...
                def _import_submodule(blade, submodule_name):
                    ...
                    if blade == 'tnz_arm':
                        ...
                        gr_dir = Path(FSI_path) / 'STRUCTURE' / 'SONATA' / blade / 'TELE_coords'
                        mod_file = gr_dir / f'{submodule_name}.py'
    ...
    for (n_span, n_chord_count), aerogrid in aerogrid_dict.items():
        ...
        out_dir = FSI_path + f"/FLUID/PanelAero/Qjj/qjj_precomputed/{blade_name}_{fluid}_alpha{attack_angle[0]}_nspan{n_span}_nchord{n_chord_count}_{DLM_method}"
        out_dir_vlm = FSI_path + f"/FLUID/PanelAero/Qjj/qjj_precomputed/vjj_{blade_name}_{fluid}_alpha{attack_angle[0]}_nspan{n_span}_nchord{n_chord_count}_klist_{DLM_method}"
...
def plot_aerogrid(FSI_path, aerogrid, le_curve, te_curve, n_span, n_chord_count, blade_name, fluid, attack_angle):
    ...
    try:
        plot_out_dir = Path(FSI_path + "/FLUID/PanelAero/Qjj/qjj_precomputed/{blade_name}_{fluid}_alpha{attack_angle[0]}_nspan{n_span}_nchord{n_chord_count}_klist_new")
